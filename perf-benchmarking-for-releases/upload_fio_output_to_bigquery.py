...
for job in data.get("jobs", []):
    ...
    clat_ns = {}
    # Fio provides both read and write sections in its JSON output.
    # For write or mixed workloads, the 'write' section will contain the
    # relevant latency data. For read-only workloads, the 'write' section will
    # have zero iops, so we fall back to the 'read' section.
    if write.get("iops", 0.0) > 0:
        clat_ns = write.get("clat_ns", {})
    else:
        clat_ns = read.get("clat_ns", {})

    # The 'percentile' key can be missing or have a null value in fio's JSON output.
    percentiles = clat_ns.get("percentile") if clat_ns else None
    if not percentiles:
        percentiles = {}
