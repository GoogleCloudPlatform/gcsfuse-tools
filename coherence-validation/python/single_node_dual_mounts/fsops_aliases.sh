...
# Script is in shared/coherency-validation/python/single_node_dual_mounts
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# PYTHON_MODULE_DIR is the parent of single_node_dual_mounts (i.e., python/)
...
try:
    from single_node_dual_mounts import config
    import json
...
_execute_scenario_internal() {
    ...
    python3 -m single_node_dual_mounts.execute_scenarios "$@"
    local ret=$?
...
alias execute_scenario_complete='_execute_scenario_internal --complete'
alias execute_scenario_stepmode='_execute_scenario_internal --stepmode'
alias list_scenarios='python3 -m single_node_dual_mounts.execute_scenarios --list'
