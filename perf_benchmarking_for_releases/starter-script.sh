...
if [[ "$OS_FAMILY" == "debian_ubuntu" ]]; then
    sudo apt-get update
    sudo apt-get install -y git fio libaio-dev gcc make mdadm build-essential python3-setuptools python3-crcmod python3-pip fuse jq bc procps
...
python3 -m pip install --user --break-system-packages -r gcsfuse-tools/perf_benchmarking_for_releases/requirements.txt
