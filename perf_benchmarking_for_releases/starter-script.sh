...
# Install common dependencies, including bc and procps for monitoring
...
for master_fio_file in "$FIO_JOB_DIR"/*.fio; do
    ...
    awk '
      /^\[global\]/ { in_global=1; global_section="" } # Start of global section
      in_global && /^\[/ && NR > 1 { in_global=0 } # End of global section (if a new section starts)
      in_global { global_section = global_section $0 "\n"; next } # Collect global section lines
      /^\[/ && !in_global { # Start of a new job section (not global)
        jobname = substr($0, 2, length($0)-2); # Extract job name (e.g., "Workload" from "[Workload]")
        sanitized_jobname = gensub(/[^a-zA-Z0-9_-]/, "_", "g", jobname); # Sanitize name for filename
        close(outfile); # Close previously opened output file
        outfile = "'"$SPLIT_DIR"'" "/" sanitized_jobname ".fio"; # Define new output filename
        printf "%s", global_section > outfile; # Write global section to new file (overwrites if exists)
      }
      !in_global { # If not in global section (i.e., in a job section)
        print >> outfile; # Append current line to the current job's file
      }
    ' "$master_fio_file"
