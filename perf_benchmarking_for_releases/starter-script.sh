...
if [[ "$OS_FAMILY" == "debian_ubuntu" ]]; then
    sudo apt-get update
    sudo apt-get install -y git fio libaio1 libaio-dev gcc make mdadm build-essential python3-setuptools python3-crcmod python3-pip fuse jq bc procps
elif [[ "$OS_FAMILY" == "rhel_centos" ]]; then
    sudo yum makecache
    sudo yum -y install git fio fuse libaio libaio-devel gcc make mdadm redhat-rpm-config python3-devel python3-setuptools python3-pip jq bc procps-ng
    pip3 install crcmod
fi
...
monitor_gcsfuse_usage() {
  ...
  # Find the gcsfuse Process ID (PID)
  local gcsfuse_pid=$(pgrep -f gcsfuse)
  if [ -z "$gcsfuse_pid" ]; then
    echo "GCS FUSE process not found. Cannot monitor."
    return 1
  fi
  echo "Monitoring gcsfuse process with PID: $gcsfuse_pid"

  # Get initial CPU stats for both the system and the process
  ...
  while ps -p "$gcsfuse_pid" > /dev/null; do
    ...
    # Check if process still exists before reading its stat file
    ...
    # Get Memory usage (VmRSS) for the specific PID in MiB
    ...
    # Update "before" stats for the next interval
...
cleanup() {
    # If the monitor process is still running, kill it.
...
# This loop processes one master FIO file at a time.
for master_fio_file in "$FIO_JOB_DIR"/*.fio; do
    ...
    awk '
      /^\[global\]/ { in_global=1; global_section="" }
      ...
      /^\[/ && !in_global {
        ...
        printf "%s", global_section > outfile;
      }
      !in_global {
        print >> outfile;
      }
    ...
    # Loop through the newly created single-job FIO files and run them.
    for single_fio_file in "$SPLIT_DIR"/*.fio; do
        ...
        # Mount GCS bucket first so we can find the gcsfuse PID
        mkdir -p "$MNT"
        "$GCSFUSE_BIN" "${GCSFUSE_FLAGS[@]}" "$GCS_BUCKET_WITH_FIO_TEST_DATA" "$MNT"
        
        monitor_log="/tmp/monitor_${job_file_basename}.log"
        
        # Start process-specific monitoring in the background
        monitor_gcsfuse_usage "$monitor_log" &
        monitor_pid=$!

        RESULT_FILE="/tmp/gcsfuse-${job_file_basename}-benchmark.json"

        # Run FIO job
        DIR="$MNT" fio "$single_fio_file" --output-format=json --output="$RESULT_FILE" 1>/dev/null
        FIO_EXIT_CODE=$?
        ...
        if [ $FIO_EXIT_CODE -ne 0 ]; then
            echo "FIO command failed for $single_fio_file. Skipping."
            rm -f "$RESULT_FILE" "$monitor_log"
            continue
        fi
        
        # Extract Min/Max from our local monitor log
