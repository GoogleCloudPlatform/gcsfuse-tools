...
1. **Regional Standard HNS Target**: `is_rapid_bucket: false`, dataset `<prefix>_regional`, Regional HNS bucket (e.g. `gs://npi-smoke-regional-*`).
2. **Zonal RAPID HNS Target**: `is_rapid_bucket: true`, dataset `<prefix>_zonal`, Zonal RAPID HNS bucket (e.g. `gs://npi-smoke-zonal-*`).
