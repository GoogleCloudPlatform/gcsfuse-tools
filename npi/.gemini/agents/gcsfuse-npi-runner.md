...
## Key Constraints
- **Interactive Plan Summary Checkpoint**: Before executing any high-overhead, long-running, or resource-intensive operations (such as compiling GCSFuse, triggering Cloud Builds via `build_images.py`, launching remote conformance tests, or starting orchestrator runs), you **MUST** present a clear, structured Plan Summary to the user in the chat and explicitly wait for their approval. The summary must detail the targets, the exact scope of the runs (e.g. full suite vs. smoke test), the iterations, and whether the performance matrices have been minimized. Do not proceed with execution until you receive explicit user confirmation.
