...
func ensureDataExists(ctx context.Context, bucketHandle *storage.BucketHandle, workerCount int) error {
	fmt.Println("Checking if benchmark data exists...")
	// Determine object size from prefix
	var size int64 = 1024 * 1024 * 1024 // Default 1 GB
	if strings.Contains(*objectNamePrefix, "1GB") {
		size = 1024 * 1024 * 1024
	} else if strings.Contains(*objectNamePrefix, "100MB") {
		size = 100 * 1024 * 1024
	} else if strings.Contains(*objectNamePrefix, "10MB") {
		size = 10 * 1024 * 1024
	}
	...
	for i := 0; i < workerCount; i++ {
		...
		eg.Go(func() error {
			...
			fmt.Printf("Object %s does not exist or is incomplete. Creating %d bytes object...\n", objectName, size)
			...
			for written < size {
				...
				if err != nil {
					writer.Close()
					return fmt.Errorf("failed writing to %s: %v", objectName, err)
				}
				written += int64(n)
			}
			if err := writer.Close(); err != nil {
				return fmt.Errorf("failed closing writer for %s: %v", objectName, err)
			}
			fmt.Printf("Successfully created object %s\n", objectName)
			return nil
		})
	}
	return eg.Wait()
...
func main() {
	...
	// Check and create data if it doesn't exist
	if err := ensureDataExists(ctx, bucketHandle, *numOfWorkers); err != nil {
		fmt.Printf("Failed to ensure benchmark data exists: %v\n", err)
		os.Exit(1)
	}
