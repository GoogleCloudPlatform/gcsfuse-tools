...
def prep_vm(target, socket_path):
    ...
    if target["type"] == "gce":
        ...
        if buffer_mount:
            ...
            code, out, _ = run_ssh_cmd(socket_path, vm_name, zone, f"df -h | grep -F {quoted_mount}")
            if code != 0:
...
def get_disk_utilization(socket_path, vm_name, zone, path):
    ...
    code, out, _ = run_ssh_cmd(socket_path, vm_name, zone, f"df -P {quoted_path} | tail -n 1 | awk '{{print $5}}' | tr -d '%' 2>/dev/null", timeout=10)
    ...
    if state[target_name]["status"] in ["PENDING", "FAILED"]:
        try:
            ...
            if target["type"] == "gce":
                ...
                python_args.extend(["--benchmarks"] + args.benchmarks.split())
