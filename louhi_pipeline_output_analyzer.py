...
def normalize_log_line(line: str) -> str:
    ...
    line = re.sub(r'/tmp/gcsfuse_readwrite_test_\d+/', '/tmp/gcsfuse_readwrite_test_XXXXXX/', line)
    # Normalize timestamps in JSON objects
    line = re.sub(r'"timestamp":\{"seconds":\d+,"nanos":\d+\}', '"timestamp":{"seconds":XXXXXXXXXX,"nanos":YYYYYYYYY}', line)
    ...
    line = re.sub(r'retry failed after \d+ attempts', 'retry failed after N attempts', line)
    # Normalize temp policy JSON file names (e.g., /tmp/iam-policy-217733623.json)
    line = re.sub(r'/tmp/iam-policy-\d+\.json', '/tmp/iam-policy-XXXXXX.json', line)
...
def parse_failing_log_content(log_content: str) -> List[Dict[str, Any]]:
    ...
    def classify_line(line):
        line = line.strip()
        if line.startswith("=== RUN"): return "test_case_start"
        if line.startswith("--- FAIL:"): return "test_case_end_failed"
        if line.startswith("--- PASS:"): return "test_case_end_successful"
        if line.startswith("--- SKIP:") or line.startswith("SKIP:"): return "test_case_end_skipped"
        if line.startswith("=== Log for"): return "package_start"
        if "Running static mounting tests with flags:" in line: return "flag_options_setting"
        if line in ["PASS", "FAIL"]: return "keyword_only"
        return "test_logs"
    ...
    for line in lines:
        ...
        if "ERROR: [otel-plugin] ctx passed into client side stats handler metrics event handling has no client attempt data present" in stripped:
            continue
        ...
        if classification == "package_start":
            ...
            if match: current_package_name = match.group(1)

        elif classification == "flag_options_setting":
            match = re.search(r'\[(.*?)\]', stripped)
            if match: current_flags = match.group(1).strip()
            ...
            if len(parts) >= 3:
                ...
                if "/" in test_name:
                    ...
                    if parent in test_map: test_map[parent]['has_subtests'] = True
        ...
        elif classification in ["test_case_end_successful", "test_case_end_failed", "test_case_end_skipped"]:
            parts = stripped.split()
            test_name = None
            if classification == "test_case_end_skipped" and stripped.startswith("SKIP:"):
                if len(parts) >= 2: test_name = parts[1]
            elif len(parts) >= 3:
                 test_name = parts[2]
            ...
            if test_name:
                ...
                if test_name in test_map:
                    test_map[test_name]["status"] = "FAIL" if classification == "test_case_end_failed" else "PASS"
    ...
    failed_leaf_tests = [t for t in test_map.values() if not t['has_subtests'] and t['status'] == 'FAIL']
    ...
    for test in failed_leaf_tests:
        ...
        while i < len(original_logs):
            ...
            if repeat_count > 0:
                ...
                compressed_logs.append(f"[repeat of previous line {repeat_count} times]")
            ...
            if i + 3 < len(original_logs):
                ...
                while k + 1 < len(original_logs):
                    ...
                    if (normalize_log_line(cand_1) == current_norm and 
                        normalize_log_line(cand_2) == next_norm):
                        block_repeats += 1
                        k += 2
                    else:
                        break
                ...
                if block_repeats > 0:
                    ...
                    compressed_logs.append(f"[repeat of previous 2 lines {block_repeats} times]")
...
def process_vm_target(bucket, vm_dir: str, suffix_key: str, bucket_type: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        raw_log_url = f"{GCS_AUTH_BASE_URL}/{BUCKET_NAME}/{version}/{vm_subpath}/{log_file}"
...
def analyze_failures(version: str):
    try:
        ...
        print("Ensure you have run `auth.authenticate_user()` or have correct credentials.")
        return pd.DataFrame()
    ...
    print(f"Found {len(vm_dirs)} VM directories. Checking for failures (Parallel)...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for vm_dir in vm_dirs:
            for suffix_key, bucket_type in TEST_TYPES.items():
                futures.append(
                    executor.submit(process_vm_target, bucket, vm_dir, suffix_key, bucket_type)
                )
    ...
    cols = ["dir_name", "bucket_type", "failing_package", "gcsfuse_flags",
            "failing_test_name", "logfile_path", "relevant_logs"]
...
def post_process(df, version_number, timestamp):
    ...
    df_md['logfile_path'] = df_md['logfile_path'].str.replace('`', '') # Clean links

    # Rename columns
    df_md.columns = ['VM', 'Bucket-type', 'Failing Package', 'GCSFuse Flags',
                     'Failing Test', 'Logfile Path', 'Relevant Logs']

    # Reorder columns: Swap 'Failing Test' and 'GCSFuse Flags'
    df_md = df_md[['VM', 'Bucket-type', 'Failing Package', 'Failing Test',
                   'GCSFuse Flags', 'Logfile Path', 'Relevant Logs']]

    # Sort
    df_md = df_md.sort_values(by=['VM', 'Bucket-type', 'Failing Package', 'Failing Test'])
    ...
    pkg_agg = df.groupby('failing_package').agg(
    ...
    pkg_agg.columns = ['Failing package', 'Number of failing tests', 'Unique list of failing Tests', 'Unique list of VMs', 'Unique list of bucket-types']
    ...
    vm_agg = df.groupby('dir_name').agg(
        num_tests=('failing_test_name', 'count'),
    ...
    vm_agg = vm_agg[['Failing VM', 'Number of failing tests', 'Unique list of failing test packages', 'Unique list of failing Tests', 'Unique list of bucket-types']]
...
def main():
    parser = argparse.ArgumentParser(description="Analyze GCSFuse release test failures.")
    parser.add_argument("version", nargs="?", help="GCSFuse version (e.g., v3.5.0)")
