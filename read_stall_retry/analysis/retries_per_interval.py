...
"""
Processes job log files to calculate and visualize the number of retries
over time.

This script reads a CSV log file containing timestamps of job retries.
The log file is expected to be at '/tmp/<job_name>-logs.csv'.
It aggregates these retries into configurable time intervals (e.g., every 5 minutes)
and outputs a new CSV file with the counts per interval. Additionally, it
generates a bar chart (PNG image) visualizing this data.

Key Features:
- Parses timestamps from the input log file located at '/tmp/<job_name>-logs.csv'.
- Groups retries into user-defined time intervals (e.g., 30s, 5m, 1h).
- Outputs aggregated retry counts to a CSV file named '<job_name>-retries.csv'.
- Generates a PNG bar chart of retries over time, saved as '<job_name>-retries.png'.

Example Usage:
  ./process_retries.py my-important-job 10m
  ./process_retries.py another-job --interval 1h

The input log file (e.g., /tmp/my-important-job-logs.csv) is expected to be a CSV
where the first column contains timestamps of retry events.
The script assumes that a header row is present in the input log file and skips it.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Processes job log files to calculate and visualize the number of retries over time. "
                    "Reads a CSV log file (first column as timestamps) from '/tmp/<job_name>-logs.csv', "
                    "aggregates retries into time intervals, outputs a new CSV, and generates a PNG bar chart.",
