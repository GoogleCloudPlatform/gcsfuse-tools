...
def generate_combined_report(metrics, output_file):
    ...
    headers = ["Matrix ID", "Test ID", "Config", "Commit","IOType|Jobs|FSize|BS|IOD|NrFiles|Direct", "Read BW (MB/s)", "Write BW (MB/s)", 
                    ...
                    "Write Min (ms)", "Write Max (ms)", "Write Avg (ms)", "Write StdDev (ms)",
                    "Write P50 (ms)", "Write P90 (ms)", "Write P99 (ms)",
    ...
    for test_key in sorted(metrics.keys()):
        ...
        row = [
            ...
            _format_metric(m.get('write_lat_min_ms', 0)),
            _format_metric(m.get('write_lat_max_ms', 0)),
            _format_metric(m.get('write_lat_avg_ms', 0)),
            _format_metric(m.get('write_lat_stddev_ms', 0)),
            _format_metric(m.get('write_lat_p50_ms', 0)),
            _format_metric(m.get('write_lat_p90_ms', 0)),
            _format_metric(m.get('write_lat_p99_ms', 0)),
...
def generate_separate_reports(metrics, base_output_file):
    ...
    headers = ["Test ID", "IOType|Jobs|FSize|BS|IOD|NrFiles|Direct", "Read BW (MB/s)", "Write BW (MB/s)",
               ...
               "Write P50 (ms)", "Write P90 (ms)", "Write P99 (ms)", "Write Max (ms)",
               "Avg CPU (%)", "Peak CPU (%)", "Avg Mem (MB)", "Peak Mem (MB)", 
               "Avg PgCache (GB)", "Peak PgCache (GB)", "Avg Sys CPU (%)", "Peak Sys CPU (%)", 
               "Avg Net RX (MB/s)", "Peak Net RX (MB/s)", "Avg Net TX (MB/s)", "Peak Net TX (MB/s)", "Iter"]
