        ...
        while [ ! -f "$MONITOR_STOP_FLAG" ]; do
            if [ -d "/proc/$GCSFUSE_PID" ]; then
                ...
                if [ -n "$PROC_STAT" ]; then
                    ...
                    SYSTEM_TIME=$(echo "$SYSTEM_STAT" | awk '{sum=0; for(i=2;i<=NF;i++) sum+=$i; printf "%.0f", sum}')
                ...
                # --- Network Stats ---
                NET_STATS=$(awk 'NR>2 {rx+=$2; tx+=$10} END {printf "%.0f %.0f", rx, tx}' /proc/net/dev 2>/dev/null || echo "0 0")
