...
"""Bucket Cleaner: Automated Cloud Run / Functions Gen 2 Service for Sweeping Orphan GCS Buckets."""
...
def check_and_clean_orphan_buckets(request: Any) -> Any:
    """HTTP entrypoint for Google Cloud Functions Gen 2 & Functions Framework."""
    logger.info("Received Bucket Cleaner invocation request.")
...
bucket_cleaner_handler = check_and_clean_orphan_buckets


@app.route("/", methods=["GET", "POST"])
def index() -> Any:
    """HTTP routing for Cloud Run container hosting."""
    return check_and_clean_orphan_buckets(flask_request)


@app.route("/healthz", methods=["GET"])
def healthz() -> Any:
    """Health check endpoint for container orchestrators."""
    return jsonify({"status": "healthy", "service": "bucket-cleaner"}), 200


if __name__ == "__main__":
    server_port = int(os.environ.get("PORT", 8080))
    logger.info("Starting Bucket Cleaner Flask server on port %d...", server_port)
