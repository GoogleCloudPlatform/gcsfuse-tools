...
# ==============================================================================
# Script: deploy.sh
# Purpose: Automates end-to-end deployment of Bucket Cleaner to Cloud Run
...
setup_service_accounts() {
  ...
  if [[ "${DRY_RUN}" == "true" ]]; then
    ...
    log_dry_run "gcloud iam service-accounts create ${RUNNER_SA_NAME} --project=${PROJECT_ID} --display-name=\"Bucket Cleaner Cloud Run Runner SA\""
  else
    if ! gcloud iam service-accounts describe "${RUNNER_SA_EMAIL}" --project "${PROJECT_ID}" >/dev/null 2>&1; then
      ...
      gcloud iam service-accounts create "${RUNNER_SA_NAME}" \
        --project "${PROJECT_ID}" \
        --display-name "Bucket Cleaner Cloud Run Runner SA" || error_exit "Failed to create Runner Service Account."
    else
  ...
  if [[ "${DRY_RUN}" == "true" ]]; then
    ...
    log_dry_run "gcloud iam service-accounts create ${SCHEDULER_SA_NAME} --project=${PROJECT_ID} --display-name=\"Bucket Cleaner Cloud Scheduler Invoker SA\""
  else
    if ! gcloud iam service-accounts describe "${SCHEDULER_SA_EMAIL}" --project "${PROJECT_ID}" >/dev/null 2>&1; then
      ...
      gcloud iam service-accounts create "${SCHEDULER_SA_NAME}" \
        --project "${PROJECT_ID}" \
        --display-name "Bucket Cleaner Cloud Scheduler Invoker SA" || error_exit "Failed to create Scheduler Service Account."
    else
...
deploy_scheduler() {
  ...
  local payload="{\"projects\":[\"gcs-fuse-test\",\"gcs-fuse-test-ml\"],\"age_days\":3,\"dry_run\":${DRY_RUN}}"
  local common_args=(
    --project "${PROJECT_ID}"
    --location "${REGION}"
